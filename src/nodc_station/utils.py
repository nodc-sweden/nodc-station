...
def add_latlon_distance_to_df(df: pl.DataFrame, lat_dd: float, lon_dd: float) -> pl.DataFrame:
    ...
    df = df.with_columns(
        lat_rad=pl.col("LATITUDE_WGS84_SWEREF99_DD").radians(),
        lon_rad=pl.col("LONGITUDE_WGS84_SWEREF99_DD").radians(),
    )

    df = df.with_columns(
        diff_lat_rad=pl.col("lat_rad") - lat_rad,
        diff_lon_rad=pl.col("lon_rad") - lon_rad,
    )
